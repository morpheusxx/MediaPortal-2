...
namespace MediaPortal.UI.SkinEngine.Controls.Panels
{
  public class CarouselControl : Canvas
  {
    ...
    protected AbstractProperty _rotationSpeedProperty = new SProperty(typeof(double), DEFAULT_ROTATION_SPEED);
    protected AbstractProperty _lookDownOffsetProperty = new SProperty(typeof(double), DEFAULT_LOOKDOWN_OFFSET);
    protected AbstractProperty _fadeProperty = new SProperty(typeof(double), DEFAULT_FADE);
    protected AbstractProperty _scaleProperty = new SProperty(typeof(double), DEFAULT_SCALE);

    private FrameworkElement _currentlySelected = null;
    public FrameworkElement CurrentlySelected { get { return _currentlySelected; } }

    protected double CenterX { get { return ActualWidth / 2.0; } }
    protected double CenterY { get { return ActualHeight / 2.0; } }
    ...
    {
      get { return (double)_rotationSpeedProperty.GetValue(); }
      set { _rotationSpeedProperty.SetValue(Math.Min(Math.Max(value, MINIMUM_ROTATION_SPEED), MAXIMUM_ROTATION_SPEED)); }
    }
    public AbstractProperty RotationSpeedProperty
    {
      get { return _rotationSpeedProperty; }
    }
    ...
    {
      get { return (double)_lookDownOffsetProperty.GetValue(); }
      set { _lookDownOffsetProperty.SetValue(Math.Min(Math.Max(value, MINIMUM_LOOKDOWN_OFFSET), MAXIMUM_LOOKDOWN_OFFSET)); }
    }
    public AbstractProperty LookDownOffsetProperty
    {
      get { return _lookDownOffsetProperty; }
    }
    ...
    public double Fade
    {
      get { return (double)_fadeProperty.GetValue(); }
      set { _fadeProperty.SetValue(Math.Min(Math.Max(value, MINIMUM_FADE), MAXIMUM_FADE)); }
    }
    public AbstractProperty FadeProperty
    {
      get { return _fadeProperty; }
    }
    ...
    public double Scale
    {
      get { return (double)_scaleProperty.GetValue(); }
      set { _scaleProperty.SetValue(Math.Min(Math.Max(value, MINIMUM_SCALE), MAXIMUM_SCALE)); }
    }
    public AbstractProperty ScaleProperty
    {
      get { return _fadeProperty; }
    }
    ...
    public void SetElementPositions()
    {
      for (int index = 0; index < TotalNumberOfElements; index++)
      {
        ...
        double degrees = 360 * ((double)index / TotalNumberOfElements) + _currentRotation;
